...
一般的なルールとして、電源配線は平均的な信号配線よりも太くする必要がある。これは特に、細い配線幅（約5ミル）を提供するハイエンドの製造プロセスを使用する場合に重要だ。電源配線は、抵抗成分による発熱と寄生成分のインダクタンスの両方に対抗するために太くしなければならない。細い電源配線、特に重要な電源分配点付近の配線は抵抗のように機能して発熱し、供給電圧を低下させ、回路に不定動作を引き起こすことがある。
